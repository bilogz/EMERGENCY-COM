<?php
/**
 * Fixed Setup Script for Enhanced Multilingual Support
 * Properly handles table and column creation
 */
...
echo "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Multilingual Setup (Fixed)</title>";
echo "<style>
...
echo "<h1>🌍 Enhanced Multilingual Support Setup (Fixed)</h1>";
echo "<pre>";
...
// Step 1: Add missing columns to alert_translations
echo "<h2>Step 1: Updating alert_translations table</h2>\n";

try {
    // Check and add translated_by_admin_id
    $stmt = $pdo->query("SHOW COLUMNS FROM alert_translations LIKE 'translated_by_admin_id'");
    if ($stmt->rowCount() === 0) {
    ...
    // Check and add translation_method
    $stmt = $pdo->query("SHOW COLUMNS FROM alert_translations LIKE 'translation_method'");
    if ($stmt->rowCount() === 0) {
    ...
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate key name') === false) {
            echo "<span class='info'>ℹ Index 'idx_translated_by' already exists or error: " . $e->getMessage() . "</span>\n";
        }
    }
    ...
    echo "<span class='error'>✗ Error updating alert_translations: " . $e->getMessage() . "</span>\n";
}
...
echo "\n<h2>Step 3: Inserting languages</h2>\n";
...
foreach ($languages as $lang) {
    try {
        ...
        if ($errors <= 5) { // Show first 5 errors
            echo "<span class='error'>✗ Error inserting {$lang[1]}: " . substr($e->getMessage(), 0, 80) . "</span>\n";
        }
    }
}

echo "<span class='success'>✓ Inserted {$inserted} languages</span>\n";
if ($updated > 0) {
    echo "<span class='info'>ℹ Updated {$updated} existing languages</span>\n";
}
if ($errors > 0) {
    echo "<span class='error'>✗ {$errors} errors occurred</span>\n";
}
...
try {
    ...
    // Check tables
    $tables = ['alert_translations', 'supported_languages', 'translation_activity_logs'];
    ...
    // Check columns
    $stmt = $pdo->query("SHOW COLUMNS FROM alert_translations");
    $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $requiredCols = ['translated_by_admin_id', 'translation_method'];
    foreach ($requiredCols as $col) {
        if (in_array($col, $columns)) {
            echo "<span class='success'>✓ Column 'alert_translations.{$col}' exists</span>\n";
        } else {
            echo "<span class='error'>✗ Column 'alert_translations.{$col}' missing</span>\n";
        }
    }
...
echo "\n<span class='success'>The enhanced multilingual support system has been set up.</span>\n";
echo "\nNext steps:\n";
echo "1. Test the language selector on user pages\n";
echo "2. Add languages via Admin → Language Management\n";
echo "3. Verify real-time updates work\n";
echo "4. Test device language detection\n";
